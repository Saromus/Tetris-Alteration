class LeftZShape extends Shape{
  final String block = "tetrispurple.jpg";
  Ground ground;
  double bsize;
  int position = 1;
  
  LeftZShape(double w,double r,Ground g){
  ...
  void createShape(double blockSize, double x, double y){
    ...
    blocks[0] = new Block(block,x - (bsize / 2),y - (bsize / 2) - bsize,blockSize,ground);
    ...
    blocks[3] = new Block(block,x - (bsize / 2) - bsize,y + (bsize / 2),blockSize,ground);
    setBlocks(blocks);
  }
}
