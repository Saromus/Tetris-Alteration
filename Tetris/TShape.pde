class TShape extends Shape{
  ...
  void rotateClockwise(){
    ...
    if(position == 1){
      ...
      blocks[2].adjustXY(getX() - bsize - (bsize / 2),getY() - (bsize / 2));
      blocks[3].adjustXY(getX() - (bsize / 2),getY() + (bsize / 2));
      position = 2;
    } else if(position == 2){
      blocks[0].adjustXY(getX() - (bsize / 2),getY() + (bsize / 2));
      blocks[2].adjustXY(getX() - (bsize / 2),getY() - bsize - (bsize / 2) );
      blocks[3].adjustXY(getX() - (bsize / 2) - bsize,getY() - (bsize / 2));
      position = 3;
    } else if(position == 3){
      blocks[0].adjustXY(getX() - bsize - (bsize / 2),getY() - (bsize / 2));
      blocks[2].adjustXY(getX() + (bsize / 2),getY() - (bsize / 2));
      blocks[3].adjustXY(getX() - (bsize / 2),getY() - bsize - (bsize / 2));
      position = 4;
    } else if(position == 4){
